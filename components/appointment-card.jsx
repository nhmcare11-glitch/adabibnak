...
import { getPrescriptionByAppointment } from "@/actions/prescriptions";
...
export function AppointmentCard({
  ...
  // جلب الوصفة عند فتح الـ Dialog
  useEffect(() => {
    if (open && !prescriptionLoaded) {
      getPrescriptionByAppointment(appointment.id).then(({ prescription }) => {
        setPrescription(prescription || null);
        setPrescriptionLoaded(true);
      });
    }
  }, [open, prescriptionLoaded, appointment.id]);
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Badge
                ...
                className={
                  appointment.status === "COMPLETED"
                    ? "bg-emerald-900/20 border-blue-900/30 text-blue-400"
                    : appointment.status === "CANCELLED"
                    ? "bg-red-900/20 border-red-900/30 text-red-400"
                    : "bg-amber-900/20 border-amber-900/30 text-amber-400"
                }
            ...
            {/* =================== TAB 2: الوصفة الطبية =================== */}
            <TabsContent value="prescription" className="py-2">
              {!prescriptionLoaded ? (
                <div className="flex justify-center py-10">
                  <Loader2 className="h-6 w-6 animate-spin text-blue-400" />
                </div>
              ) : userRole === "DOCTOR" && appointment.status !== "CANCELLED" ? (
                <PrescriptionForm
                  appointmentId={appointment.id}
                  existingPrescription={prescription}
                  onSaved={(saved) => setPrescription(saved)}
                />
              ) : (
