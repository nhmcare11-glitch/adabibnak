...
import {
  approveDoctorVM,
  rejectDoctorVM,
} from "@/actions/verification-manager";
...
function StatCard({ label, value, icon: Icon, color }) {
  const colors = {
    teal: {
      bg: "bg-[#2DBFB8]/10 dark:bg-[#2DBFB8]/15",
      text: "text-[#2DBFB8]",
      border: "border-[#2DBFB8]/20",
    },
    green: {
  ...
  return (
    <div
      className={`rounded-2xl border ${c.border} bg-white/60 dark:bg-[#0a1e1d]/80 backdrop-blur p-5 flex items-center gap-4`}
    >
      <div
        className={`w-12 h-12 rounded-xl ${c.bg} flex items-center justify-center shrink-0`}
      >
        <Icon className={`w-6 h-6 ${c.text}`} />
      </div>

      <div>
        <p className="text-2xl font-black text-[#062220] dark:text-white">
          {value}
        </p>

        <p className="text-sm text-[#54706d] dark:text-slate-400">
          {label}
        </p>
      </div>
    </div>
    ...
    <div className="rounded-2xl border border-[#2DBFB8]/15 dark:border-[#2DBFB8]/20 bg-white/70 dark:bg-[#0a1e1d]/80 backdrop-blur overflow-hidden transition-all duration-300 hover:border-[#2DBFB8]/40 hover:shadow-lg hover:shadow-[#2DBFB8]/5">
      ...
      <div className="p-5 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="font-bold text-[#062220] dark:text-white truncate">
              {doctor.name || "غير محدد"}
            </h3>

            <span
              className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColors[doctor.verificationStatus]}`}
            >
              {statusLabels[doctor.verificationStatus]}
            </span>
          </div>

          <p className="text-sm text-[#54706d] dark:text-slate-400 mt-0.5">
            {doctor.email}
          </p>

          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            {doctor.specialty && (
              <span className="text-xs bg-[#2DBFB8]/10 text-[#2DBFB8] px-2 py-0.5 rounded-full">
                {doctor.specialty}
              </span>
            )}

            {doctor.experience && (
              <span className="text-xs text-[#54706d] dark:text-slate-400">
        ...
        <button
          ...
          className="text-[#54706d] hover:text-[#2DBFB8] transition-colors shrink-0"
        >
          {expanded ? (
      ...
      {expanded && (
        <div className="px-5 pb-4 border-t border-[#2DBFB8]/10 pt-4 space-y-3">
          {doctor.description && (
            <div>
              <p className="text-xs text-[#54706d] dark:text-slate-400 mb-1">
                وصف الخدمات
              </p>

              <p className="text-sm text-[#062220] dark:text-slate-200 leading-relaxed">
                {doctor.description}
              </p>
            </div>
          )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl border border-[#2DBFB8]/20 text-sm text-[#54706d] hover:bg-[#2DBFB8]/5 transition-all"
                >
                  إلغاء
                </button>
              </div>
...
export default function VerificationManagerDashboard({
  ...
  const filtered = currentDoctors.filter(
    (d) =>
      !search ||
  ...
  return (
    <div
      className="min-h-screen bg-[#eefdff] dark:bg-[#071312] transition-colors duration-500"
      dir="rtl"
    >
      {/* Header */}
      <div className="border-b border-[#2DBFB8]/10 bg-white/60 dark:bg-[#071817]/80 backdrop-blur sticky top-0 z-30">
        <div className="container mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-black text-[#062220] dark:text-white text-lg leading-none">
              لوحة التحقق
            </h1>

            <p className="text-xs text-[#54706d] dark:text-slate-400">
              مراجعة وقبول الأطباء
            </p>
          </div>
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
          ...
          <div className="flex gap-1 bg-white/60 dark:bg-[#0a1e1d]/80 border border-[#2DBFB8]/15 rounded-2xl p-1">
            {tabs.map((tab) => (
              ...
              >
                ...
                <span
                  className={`text-xs ${
                    activeTab === tab.key ? "text-white/80" : tab.color
                  }`}
                >
                  ({tab.count})
                </span>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="mr-auto">
            <SignOutButton>
              <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-sm font-bold transition-all">
