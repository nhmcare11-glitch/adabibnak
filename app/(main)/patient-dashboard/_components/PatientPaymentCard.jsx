...
export default function PatientPaymentCard({ payment, appointment }) {
    ...
    return (
      <div className="ppc-wrap">
        ...
        <div className="ppc-card">
          ...
          <p className="ppc-desc">سيراجع طبيبك هذا الاقتراح ويوافق على طريقة الدفع المناسبة.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="ppc-wrap">
        ...
        <div className="ppc-card ppc-paid">
          ...
          {paidAmount > 0 && <div className="ppc-amount-display">{paidAmount?.toLocaleString("ar-DZ")} دج</div>}
          <p className="ppc-desc muted">وافق عليه: {approvedBy?.name || "الطبيب"}</p>
        </div>
      </div>
    );
  }
  ...
  if (status === "APPROVED") {
      ...
      return (
        <div className="ppc-wrap">
          ...
          <div className="ppc-card">
            ...
            {paidAmount > 0 && <div className="ppc-amount-display">{paidAmount?.toLocaleString("ar-DZ")} دج</div>}
            <p className="ppc-desc">يرجى دفع المبلغ نقداً عند الاستشارة أو وفق تعليمات الطبيب.</p>
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="ppc-wrap">
          ...
          <div className="ppc-card">
            ...
            {paidAmount > 0 && <div className="ppc-amount-display">{paidAmount?.toLocaleString("ar-DZ")} دج</div>}
            <p className="ppc-desc">سيتم تحديد موعد وطريقة الدفع لاحقاً بالتنسيق مع الطبيب.</p>
          </div>
        </div>
      );
    }

    // ─── CARD: PayPal + Chargily ───
    if (approvedMethod === "CARD") {
      ...
      const isAnyLoading = isPending || chargilyLoading;
      ...
      return (
        <div className="ppc-wrap">
          ...
          <div className="ppc-card">
            ...
            <div className="ppc-suggestion">
              <span className="ppc-label">طريقة الدفع:</span>
              <span className="ppc-method">💳 أونلاين</span>
            </div>

            {/* حقل المبلغ — يظهر فقط إذا لم يكن محدداً */}
            {!paidAmount && (
              <div className="ppc-amount-input-wrap">
                <label className="ppc-input-label">المبلغ (دج)</label>
                <input
                  type="number"
                  min="1"
                  step="1"
            ...
            {paidAmount > 0 && (
              <div className="ppc-amount-display">{paidAmount?.toLocaleString("ar-DZ")} دج</div>
            )}

            <p className="ppc-choose-label">اختر طريقة الدفع:</p>

            {/* ── زر Chargily ── */}
            <button
              className="ppc-chargily-btn"
              ...
              {chargilyLoading ? (
                ...
                <>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                    ...
                    <path d="M8 12h8M14 8l4 4-4 4" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  EDAHABIA / CIB — Chargily
                </>
              )}
            </button>
...
const cardStyles = `
  ...
  .ppc-input {
    ...
    color: #fff; padding: 0.65rem 1rem; font-size: 1rem;
    outline: none; box-sizing: border-box;
  }
  .ppc-input:focus { border-color: #38bdf8; }
  .ppc-choose-label {
    font-size: 0.82rem; color: #64748b;
    margin: 1rem 0 0.6rem; text-align: center;
  }
  .ppc-chargily-btn {
    width: 100%;
  ...
  .ppc-chargily-btn:hover  { opacity: 0.9; transform: translateY(-1px); }
  .ppc-chargily-btn:disabled { opacity: 0.5; cursor: not-allowed; }
  .ppc-paypal-btn {
    width: 100%;
    ...
    padding: 0.85rem; font-size: 0.95rem; font-weight: 600;
    cursor: pointer; display: flex; align-items: center;
    justify-content: center; gap: 0.6rem; margin-top: 0;
    transition: opacity 0.2s, transform 0.1s;
    letter-spacing: 0.02em;
  }
  ...
  .ppc-error {
    ...
    color: #fca5a5; font-size: 0.85rem; margin-top: 0.8rem;
  }
`;